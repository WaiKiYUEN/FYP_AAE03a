...
class MoveToPos():
    ...
    def GotoLocalPos(self, x, y, z, yaw):
        ...
        while ((np.linalg.norm(np.array(self.local_pos[0:3]) - np.array([x, y, z])) > 0.1) or (abs(yaw - self.local_pos[3]) > 0.01)):
            ...
            if (self.UAV_state.mode != "OFFBOARD" and (rospy.Time.now() - last_request > rospy.Duration(5.0))):
                self.set_mode(0, 'OFFBOARD')
                print("uav enabling offboard mode")
                last_request = rospy.Time.now()
            else:
                if (not self.UAV_state.armed and (rospy.Time.now() - last_request > rospy.Duration(5.0))):
    ...
    def GotoGlobPos(self, req):
        ...
        dist = self.GotoLocalPos(x2 - x1, y2 - y1, alt - self.global_pos[2], yaw, 0.1)
