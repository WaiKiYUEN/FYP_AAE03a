...
class image_convert_pub:
  ...
  def detect_aruco(self,img):
    ...
    #default dict and size of a big marker are (5x5_250_id122) and 0.55
    if self.close == 0:
      aruco_dict = aruco.Dictionary_get(aruco.DICT_5X5_250) #6x6_250_id18 for small
      markerSize = 0.55 #0.08 for small
    else:
    ...
    """
    if ids == 122 or ids == 18:
      pls = cv2.aruco.estimatePoseSingleMarkers(corners, markerSize, self.K, self.distCoeffs)
      rvecs = pls[0]
      tvecs = pls[1]
