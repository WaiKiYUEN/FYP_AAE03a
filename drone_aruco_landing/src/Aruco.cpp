...
int main(int argc, char **argv){
    ...
    ros::NodeHandle nh;
    ros::Subscriber camera_info_sub = nh.subscribe("/camera/color/image_raw/camera_info",1,camera_info_cb);
    ros::Subscriber camera_rgb_sub = nh.subscribe<CompressedImage>("/camera/color/image_raw/compressed",1,camera_rgb_cb);
    ros::Publisher ArucoPose_pub = nh.advertise<geometry_msgs::PoseStamped>("/aruco_marker_pos",1);
